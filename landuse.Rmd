...
# land use years
yrs1 <- unique(dat$name) %>% 
  sort %>% 
  .[1:(length(.) - 1)]

# subtidal years
yrs2 <- unique(subtdat$name) %>% 
  sort %>% 
  .[1:(length(.) - 1)]
...
trndtab1 <- reactive({
  ...
  out <- lulcrct_fun(sums, 'Category', grpby = F, yrsel = yrsel1)
...
trndtab2 <- reactive({
  ...
  out <- subtrct_fun(sums, 'Category', yrsel = yrsel2)
...
cmptab1 <- reactive({
  ...
  out <- cmprctfun2(datin, fluccs, yrsel1)
